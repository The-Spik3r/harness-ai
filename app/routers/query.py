...
def _handle_query(request: QueryRequest, identity: Identity) -> QueryResponse:
    """The route's entire former body (PRD-010 STORY-006 / PRD Risk 2).

    Runs on the dedicated pipeline executor, awaited via `run_in_pipeline` from
    `query()` above -- never on anyio's shared threadpool, the one `/health`
    and every other route also draw from. `HTTPException` raised in here is
    caught by none of `query()`'s `except` arms, so it propagates through the
    executor's future unchanged and reaches the client exactly as it did when
    this logic lived directly in the route.
    """
    ...
    # the pipeline through one chain of `except` arms. That chain now lives in
    # `query()`, wrapping the whole `await run_in_pipeline(...)` call, so the
    # same mapping still applies -- `HTTPException` is caught by none of those
    # arms, so the 403 below still reaches the client unchanged.
