...
- [x] Trigger upstream/internal error, click Retry → re-submits original prompt unchanged via `send()`
- [x] Trigger duplicate block, click "Edit and resend" → repopulates composer and focuses without sending, showing `DUPLICATE_CHANGE_NOTICE`
- [x] Verify `pending` guard ignores recovery actions while request is in-flight
- [x] Verify recovery actions use stored `prompt` field on message
...
- [x] Given `pending` is `True`, when a Retry or edit-and-resend action is triggered, then it is ignored — the single in-flight guard from [[STORY-003]] applies to recovery actions too.
...
- [x] Given any recovery action, when it runs, then it uses the `prompt` field stored on the message ([[STORY-005]]) — no re-derivation from rendered bubble text.
