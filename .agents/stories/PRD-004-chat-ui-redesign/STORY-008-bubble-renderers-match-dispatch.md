...
- [ ] Given `chat_ui/chat_ui/components/bubbles.py`, when it is created, then it exposes one renderer per `kind` — `user`, `assistant`, `duplicate`, `injection`, `upstream_error`, `internal_error` — and no two semantically different outcomes share a bubble style (PRD Section 11: "6 / 6 outcomes with a dedicated rendering").
- [ ] Given `components/chat.py`, when a message is rendered, then dispatch is a single `rx.match` on `message.kind`, replacing the nested `rx.cond` over three roles at [chat.py:8-51](../../../chat_ui/chat_ui/components/chat.py), so a seventh outcome later is one new arm rather than another nesting level.
- [ ] Given a `duplicate` message, when rendered, then it uses benign-nudge styling; given an `injection` message, then it uses security-event styling and displays the matched `pattern` value — the two must be visually distinct, not today's shared amber bubble.
- [ ] Given an `upstream_error` message, when rendered, then it is an upstream-incident card that names OpenRouter as the failing party, visually distinct from the `internal_error` card (Risk 7).
- [ ] Given any error card, when rendered, then it shows `detail` (the exception text) — the same strings `POST /query` already returns in its HTTP 500/502 `detail` field, so no new disclosure is introduced (PRD Section 9).
